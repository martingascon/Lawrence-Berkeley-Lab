...
switch (argc)
	...
	case 1: MsgError();
		exit(1);
		break;
	case 2: sprintf(file,"%s.Spe",argv[1]);		// FILENAME 	
		break;						 
	case 3: sprintf(file,"%s.Spe",argv[1]);		// FILENAME REB
		reb=atoi(argv[2]);				//REB = REBINING OF FOR EACH FILE (DEFAULT 1)
		break;
	case 4: sprintf(file,"%s.Spe",argv[1]);		// FILENAME A B   
		a=atoi(argv[2]);				// A AND B DEFINE THE RANGE TO FIT
		b=atoi(argv[3]);		
		break;
	case 5: sprintf(file,"%s.Spe",argv[1]);		// FILENAME A B   
		a=atoi(argv[2]);				// A AND B DEFINE THE RANGE TO FIT
		b=atoi(argv[3]);
		reb=atoi(argv[4]);	
		break;
	case 6: sprintf(file,"%s.Spe",argv[1]);		// FILENAME A B   
		a=atoi(argv[2]);				// A AND B DEFINE THE RANGE TO FIT
		b=atoi(argv[3]);
		c=atoi(argv[4]);				
		d=atoi(argv[5]);
		break;
	case 7: sprintf(file,"%s.Spe",argv[1]);		// FILENAME A B   
		a=atoi(argv[2]);				// A AND B DEFINE THE RANGE TO FIT
		b=atoi(argv[3]);
		c=atoi(argv[4]);				
		d=atoi(argv[5]); 		
		reb=atoi(argv[6]);	
		break;
	case 8: sprintf(file,"%s.Spe",argv[1]);		// FILENAME A B   
		a=atoi(argv[2]);				// A AND B DEFINE THE RANGE TO FIT
		b=atoi(argv[3]);
		c=atoi(argv[4]);				
		d=atoi(argv[5]);		
		...
		f=atoi(argv[7]);		
		break;
	case 9: sprintf(file,"%s.Spe",argv[1]);		// FILENAME A B   
		a=atoi(argv[2]);				// A AND B DEFINE THE RANGE TO FIT
		b=atoi(argv[3]);
		c=atoi(argv[4]);				
		d=atoi(argv[5]);		
...
TF1 *e1    = new TF1("e1",Background_expb,a*0.9,b*1.5,3);
TF1 *e1b   = new TF1("e1b",a*0.95,a*1,3);
...
switch (argc)   
	...
	case 4:	{
		...
		if(!*in2)
			...
			H->Fit(g1b,"NQR+");	
			g1b->GetParameters(&parg1[0]);
			H->Fit(e1b,"NQR+");
			e1b->GetParameters(&pare1[0]);
			eg1->SetParameters(pare1[0],pare1[1],pare1[2],parg1[0],parg1[1],parg1[2]); 
			cout << pare1[0] << " " << pare1[1] << " " << pare1[2] << " " << parg1[0] << " " << parg1[1] << " " << parg1[2] << endl;
			total1->SetParameters(pare1[0],pare1[1],pare1[2],parg1[0],parg1[1],parg1[2]);
			...
			//eg1->SetParameters(280,-4.80e-01,7.22e+01,8.92e+01,2.976e+01,9.024e+02);  
...
//			for (i=0;i<5;i++)   
		...
		do
			...
			ndf= total1->GetNDF();		
			chiondf=chis/ndf;
			//if (con==1) chiondf_o=chiondf;
			/*if (chiondf<chiondf_o*1.1)
				chiondf_o=chiondf;
			else
			    {
...
/*		me[0] = total1->GetParameter(3); 
		...
		break;
	case 6:	{
		total2->SetParLimits(3, 0, 100000);
		H->Fit(g1,"NQR+");
		...
		//par2 = {280,-4.80e-01,7.22e+01,8.92e+01,2.976e+01,9.024e+02};   	//for Ba-133 PV
		...
		//par2 = {}; 	//for Eu-152 PV
		//total2->SetParameters(1.544e+04,-2.11e-03,3.259e+04,1.269e+02,4.58e+01,9.57e+03,3.72e+02,8.16e+0);	
		do  
