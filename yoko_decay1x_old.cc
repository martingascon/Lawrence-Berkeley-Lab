...
Float_t datay[N],x[N],y[N],ysum[N],yn[N],d,e,f,g,h,l,tp=0,la=0,lb=0,zero=0,HRes0=0,HRes1=0,HRes=0,HOff=0,HOff1=0, max[N],area[N],bs=0,aux=0,Aprom=0,DTprom=0,eprom=0.2,dy[2000];
...
if(!*in){
	...
	for (j=ini;j<(fin+1);j++) 	
		...
		*in >> ci;						// cout << ci << endl;  //read block line
		...
		//bs=aux/100; 	cout << " BS " << bs << endl;
		bs=0;				  // Baseline averaging 100 channels
		...
		for (i=0;i<(N-1);i++)                                 
			...
			y[i]=-datay[i]*1000-bs;//*1000+bs;//+bs;	
			//cout << " data = "<< y[i] << endl;			
		...
		for (i=250;i<(260);i++)  if (yn[i]<0.1) pulTrue=0; 
		for (i=300;i<2500;i++) if (yn[i]>0.1) pulTrue=0; 
...
/*
	...
	TF1 *f1 = new TF1("f1",fitf3,la+0.1,lb);
	f1->SetParameter(0,0);	        f1->SetParameter(1,0.1);		f1->SetParameter(2,-0.0252);	
 	f1->SetParameter(3,0.1);	f1->SetParameter(4,-0.2);	
			
	gr1->Fit("f1","R");
	d=f1->GetParameter(1);  	e=f1->GetParameter(2);  e=-1/e;	f=f1->GetParError(1); 	g=f1->GetParError(2);
	
	h=f1->GetParameter(4);  	l=f1->GetParError(4);  h=-1/h;
	if (g<0.01) g=0.01;
*/
...
//////////////////////////////////////////////////////////// total function fit
	...
	TF1 *f3 = new TF1("f3",fitf3,la+0.05,lb); 
	f3->SetParameter(0,0);	        
	f3->SetParameter(1,d);				
	f3->SetParameter(2,e);
	f3->SetParameter(4,h);				
	...
	f3b=f3->GetParameter(1);   	f=f1->GetParError(0); 	
	...
	f3c= 1/f3c; 
	f3e= 1/f3e; g=f1->GetParError(1);
	//cout << "f3a= "<< f3a << " f3b = "<< f3b << " f3c= "<< f3c << " f3d = "<< f3d << " f3e= "<< f3e << endl;
	cout << " Decay times = " << f3c << " us " << " and  " << f3e << " us " <<endl; 
...
//	TF1 *f4 = new TF1("f4",fitf3,rta,lb); 
	...
	tx->DrawLatex(0.5,0.7,Form("#tau_{1} = (%.3f #pm %.2f)  #mus",e,g) );

	tx->DrawLatex(0.5,0.6,Form("#tau_{2} = (%.3f #pm %.2f)  #mus",h,l));
	c1->cd(3);
	H->Draw();// H->Rebin(125);
	...
	sprintf(ar,"#tau_{SrI2}^{MPV}= (%.2f #pm %.2f) #mus",d,e);	// Introduce una leyenda con datos
