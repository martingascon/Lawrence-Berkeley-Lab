...
char file1[100],Scint[100]="NaCl",Doped[100]=" ",title[100]="";
set_plot_style();
...
// when using protons
Float_t Mx=GetMax(x,l); Float_t mx=GetMin(x,l);  Float_t My=GetMax(y,l);  Float_t my=GetMin(y,l);
...
//H->GetXaxis()->SetRangeUser(0.01,1400);
//H->GetYaxis()->SetRangeUser(0.1,400);
H->Draw("colZ");
c1-> SetLogz();
PrintLatex2(Form("%s%s ",Scint,Doped),0.7,0.25,kBlue);

#if SECONDARIES	 
PrintLatex2(Form("Primary electrons (%2.1f %)",(l-m)/float(l)*100.0),0.35,0.95,1);
...
//Hs->GetXaxis()->SetRangeUser(0.01,1400);
//Hs->GetYaxis()->SetRangeUser(0.1,400);
Hs->Draw("colZ");
PrintLatex2(Form("%s%s ",Scint,Doped),0.7,0.85,kBlue);
PrintLatex2(Form("Secondary electrons (%2.1f %)",m/float(l)*100.0),0.30,0.95,1);
c1->cd();
#endif 	
...
Float_t Emean[nbins];
	...
	TCanvas *c3 = new TCanvas("c3","Profile in Y",40,40,800,500);
	c3->SetLogy();
	c3->SetLogx();
	//c3->Divide(3,3);
	c3->Divide(2,1);
...
/*		if ((Emean[i]>0.0105)&&(Emean[i]<0.0106)){c3->cd(1); gPad->SetLogx();py->Draw();PrintLatex2("10 eV",0.8,0.8,2);}
		...
		if ((Emean[i]>0.0500)&&(Emean[i]<0.0515)){c3->cd(3); gPad->SetLogx();py->Draw();PrintLatex2("50 keV",0.8,0.8,2);}
		...
		if ((Emean[i]>100.00)&&(Emean[i]<103.00)){c3->cd(9); gPad->SetLogx();py->Draw();PrintLatex2("100 keV",0.78,0.8,2);}
*/

 		if ((Emean[i]>10.100)&&(Emean[i]<10.200)){c3->cd(1); gPad->SetLogx();py->Draw();PrintLatex2(Form("10 keV"),0.7,0.8,2);} 
		PrintLatex2(Form("%s:%s ",Scint,Doped),0.65,0.85,kBlue);py->GetXaxis()->SetRangeUser(0.45,90);
		if ((Emean[i]>100.00)&&(Emean[i]<103.00)){c3->cd(2); TH1D* py2 = (TH1D*)py->Clone();
		py2->SetMarkerColor(kRed);
		py2->SetLineColor(kRed);
		py2->SetMarkerStyle(8);
		gPad->SetLogx();py2->Draw();PrintLatex2(Form("100 keV"),0.68,0.8,2);
		PrintLatex2(Form("%s%s ",Scint,Doped),0.65,0.85,kBlue); 
		py2->GetXaxis()->SetRangeUser(0.2,11);	}
...
PrintLatex2("Averaged",0.7,0.80,4);
#endif

////////////////////////////////////////////////////////////////////////////////////////////

#if NIST         /// NIST/G4 comparison
	Float_t aa[500],a2[500],a22[500],a23[500],a24[500];
	ifstream *in2 = new ifstream(Form("./NIST/%s_wo.dat",Scint));
	TH2F *H3 = new TH2F("H3","file2",nbins,xbins,nbins,ybins); 
	if(!*in2) cout << " ERROR OPENING FILE " <<  file2 << endl; 
	else    {
		i=0;
		while ((!in2->eof())&&(i<500))
			{
			*in2 >>  aa[i] >> a2[i] >> a22[i] >> a23[i] >> a24[i]; 
			aa[i]=aa[i]*1000;		// to convert them in keV
			H3->Fill(aa[i],a23[i]);
			i++;
			}
		delete in2;
		...
		H3->Draw("same");
		PrintLatex2(Form("%s:%s-NIST",Scint,Doped),0.70,0.72,kViolet+1);
		}
#endif
