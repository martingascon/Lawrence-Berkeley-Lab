...
/* Cs-137 Energies (intensity %): 662 keV (85 %) + 32 keV (k-Alpha from Ba-137) */ 
/* Cd-109 Energies (intensity %): 88 keV (4%), 25 keV (18 %), 22 keV (84%)  */
/* Mn-54  Energies (intensity %): 835 keV (100 %) */ 
/* Am-241 Energies (intensity %): 59.5 keV (36 %) */ 
/* Na-22  Energies (intensity %): 511 keV (annih) + 1.275 MeV */ 
/* Eu-152 Energies (intensity %): 121.8 keV (62%), 244.7 (8.4%) 344 (28%) 778.9 (13%) 994.1 (14.5%), 1112 (13.4%) 1408 (21%)  */
/* Eu-152 X-rays:  Kα2  39,5229 (55,25%)  Kα1  40,1186 (100)- Kβ3=45,289 Kβ1= 45,413  Kβ5 45,731 (31,23% total) Kβ2 46,575 Kβ4 46,705 (8,06%) */
/* Co-57  Energies (intensity %): 13.9 keV (9.54 %) 122 keV (85.6 %) 136 keV (10.6 %) */
/* Ba-133 Energies (intensity %): 31 keV (0.969), 35 keV (22.6%), 81 keV (34%), 276 (7%), 303 (18%), 356 (62%), .383 (9%)*/
...
Float_t me[3],si[3],dm[3],ds[3],data[N],integral=0,r[3],dr[3],days=0,ndf=0,chis=0,chiondf=0,chiondf_old=1000,rang=0; //[n] refers to a vector 		      
Char_t file[100],title[150]="",res,res0; 	//ci[100]="";               			//title is a string with 150 characters
Double_t rana,ranb,ranc,rand,signo=0;
Int_t FIT=1;
Char_t sample[100]="00000";
...
Int_t Ba133=0,Cs137=1,Na22=0,Co60=0,Am241=0,Eu152=0,Cd109=0,Co57=0;

if ((strstr(argv[1],"Ba133")!=NULL)) Ba133=1;
if ((strstr(argv[1],"Cs137")!=NULL)) Cs137=1;
if ((strstr(argv[1],"Co60")!=NULL))  Co60=1;
if ((strstr(argv[1],"Am241")!=NULL)) Am241=1;
if ((strstr(argv[1],"Eu152")!=NULL)) Eu152=1;
if ((strstr(argv[1],"Cd109")!=NULL)) Cd109=1;
if ((strstr(argv[1],"Co57")!=NULL))  Co57=1;
...
TF1 *eg1   = new TF1("eg1",fit_egausb,a*0.9,b*1.1,6);
TF1 *g1    = new TF1("g1","gaus",a*0.9,b*1.1);
//TF1 *g1b    = new TF1("g1b","gaus",a,b);
//TF1 *e1    = new TF1("e1",Background_exp,a*0.9,b*1.1,2);
TF1 *e1    = new TF1("e1",Background_expb,a*0.9,b*1.5,3);             //new


TF1 *eg2   = new TF1("eg2",fit_egaus2,a,d,8);
TF1 *g2    = new TF1("g2","gaus",c*0.9,d*1.1);
TF1 *e2    = new TF1("e2",Background_expb,a*1,d*1.1,3);
...
switch (argc)   
	...
	case 2:	{
		...
		PrintLatex2(Form("- %s",sample),0.25,0.95,2);
		}
		break;
	...
	case 4:	{
		...
		if(!*in2)
			...
			rang=(b+a)/2;
 			if (rang>1&&rang<1001) eg1->SetParameters(300,-0.00026,1485,parg1[0],parg1[1],parg1[2]); 
			if (rang>1000&&rang<2001) eg1->SetParameters(300,-0.00026,1485,parg1[0],parg1[1],parg1[2]); 
			if (rang>2000&&rang<3001) eg1->SetParameters(90,-0.001,2080,parg1[0],parg1[1],parg1[2]); 
			if (rang>3000&&rang<4001) eg1->SetParameters(30,-0.003,3000,parg1[0],parg1[1],parg1[2]); 
			if (rang>4000&&rang<5001) eg1->SetParameters(15,-0.004,4000,parg1[0],parg1[1],parg1[2]); 
		...
		do
			...
			if (chiondf<chiondf_o)
				chiondf_o=chiondf;
			else
			    {
			    a=a-signo*1*int(rana);
			    b = b-1*signo*int(ranb);
			    total1->SetRange(a,b);
			    }
			con++;
		...
		if (rang>4000&&rang<5000) H->GetXaxis()->SetRangeUser(2000,8000);
		if (rang>5000&&rang<8200) H->GetXaxis()->SetRangeUser(6000,8200);
		}
		break;
	case 6:	{
		Float_t li=0,ls=0,wi=0,ws=0;
		...
		if(!*in3)
			...
			rang=(b+a)/2;
 			if (rang>   1&&rang<1001) eg2->SetParameters(300,-0.0005,185,parg1[0],parg1[1],parg1[2],parg2[0],parg2[1],parg2[2]); 
			if (rang>1000&&rang<2001) eg2->SetParameters(300,-0.0005,185,parg1[0],parg1[1],parg1[2],parg2[0],parg2[1],parg2[2]);  
			if (rang>2000&&rang<3001) eg2->SetParameters(90,-0.001,180,parg1[0],parg1[1],parg1[2],parg2[0],parg2[1],parg2[2]); 
			if (rang>3000&&rang<4001) eg2->SetParameters(30,-0.003,600,parg1[0],parg1[1],parg1[2],parg2[0],parg2[1],parg2[2]); 
			if (rang>4000&&rang<5001) eg2->SetParameters(15,-0.004,1000,parg1[0],parg1[1],parg1[2],parg2[0],parg2[1],parg2[2]);/* */
			rang=(d+a)/2;
			/*if (rang>   1&&rang<1001) eg2->SetParameters(40,-0.002,620,740,(c+d)/2,20,380,(a+b)/2,20); 
			if (rang>1000&&rang<2001) eg2->SetParameters(300,-0.0005,185,parg1[0],parg1[1],parg1[2],parg2[0],parg2[1],parg2[2]);  
			if (rang>2000&&rang<3001) eg2->SetParameters(90,-0.001,180,parg1[0],parg1[1],parg1[2],parg2[0],parg2[1],parg2[2]); 
			if (rang>3000&&rang<4001) eg2->SetParameters(30,-0.003,600,740,(c+d)/2,20,380,(a+b)/2,20); 
			if (rang>4000&&rang<5001) eg2->SetParameters(15,-0.004,1000,7,(c+d)/2,20,380,(a+b)/2,20); 
			if (rang>5000&&rang<8001) eg2->SetParameters(16,-0.004,3000,7,(c+d)/2,20,388,(a+b)/2,20); 
			*/
			//par2[0]=15;par2[1]=-0.004;par2[2]=a;par2[3]=1200;par2[4]=(c+d)/2;par2[5]=100;par2[6]=380;par2[7]=par2[4]*0.95;par2[8]=par2[5]*1.1;
			}
		...
		//par2[0]=20;par2[1]=-0.00065;par2[2]=a/2;par2[3]=1200;par2[4]=(c+d)/2;par2[5]=100;par2[6]=380;par2[7]=par2[4]*0.95;par2[8]=par2[5]*1.1; // here
		par2[0]=5;par2[1]=-2.76e-02;par2[2]=7.85e+02;par2[3]=1200;par2[4]=(c+d)/2;par2[5]=100;par2[6]=380;par2[7]=par2[4]*0.95;par2[8]=par2[5]*1.1;
		...
		do 	{
			...
			if (Ba133) 
				{
				cout << " Fitting Ba-133 ..... "  << endl;
				li=par2[4]*0.083; ls=par2[4]*0.86;	// limits for the peak of 302 keV in regard with 356
				wi=par2[5]*1.0; ws=par2[5]*1.1;
				total2->SetParLimits(0, 1, 100);
				total2->SetParLimits(1, -0.5, -0.000001);
				total2->SetParLimits(2, 10, 10000);
				total2->SetParLimits(3, 1, 10000);
				total2->SetParLimits(4, 10, 10000);
				total2->SetParLimits(5, 1, 100);
				total2->SetParLimits(6, 1, 10000);
				total2->SetParLimits(7, li, ls);   	
				total2->SetParLimits(8, wi, ws);	

				}
			
			if (Cs137) 
				{	
				cout << " Fitting Cs-137 ..... "  << endl;
				li=par2[4]*0.935; ls=par2[4]*0.975;	// limits for the peak position
				wi=par2[5]*1;     ws=par2[5]*1.12; 	// limits the resolution 
				total2->SetParLimits(0, 1, 100);
				total2->SetParLimits(1, -0.5, -0.000001);
				total2->SetParLimits(2, 10, 10000);
				total2->SetParLimits(3, 1, 10000);
			...
			if (Eu152) 
				{
				cout << " Fitting Eu-152 ..... "  << endl;
				li=par2[4]*0.083; ls=par2[4]*0.90;	// limits for the peak of 302 keV in regard with 356
				wi=par2[5]*1.0; ws=par2[5]*1.1;
				total2->SetParLimits(0, 1, 100);
				total2->SetParLimits(1, -0.5, -0.000001);
				total2->SetParLimits(2, 10, 10000);
		...
		if (par2[8]>par2[5]*1.15) cout << " BE CAREFULL =>  WIDTH OF ESCAPE PEAK IS MAXIMUM " << endl;
		}
		break;
	case 8: {
		H->Fit(g1,"NQR+");
...
Float_t Energy=0;
if (Cs137)
	{if (rang<1000) Energy=32; if (rang>1000) Energy=662;}
if (Na22)
	{if (rang<5500) Energy=511; if (rang>5000) Energy=1274;}
if (Cd109)
	{Energy=122; if (rang>5000) Energy=1274;}
...
#if GRAPH
/*cout << "Do you want to save your PLOT (y/n)?" << endl;  
cin >> res0;
if (res0=='s'||res=='y')
	{  
...
//		*dat << par1[0] << "\t" << par1[1] << "\t" << par1[2] << "\t" << par1[3] <<  "\t" << par1[4] << endl;
		*out <<  std::fixed << std::setprecision(1) << argv[1][0] << argv[1][1] << "\t"  << 1 << "\t" << std::fixed << std::setprecision(1) << me[0] << "\t" << dm[0]*15 <<"\t" <<  r[0] << "\t" << dr[0]*5 << endl;	
		//*dat << par1[0] << "\t" << par1[1] << "\t" << par1[2] << "\t" << par1[3] <<  "\t" << par1[4] << "\t" << par1[5] << endl;
		break;
    		case 6:	
			//ofstream *out2 = new ofstream("data2.txt",ios::app);
	   		*out <<  std::fixed << std::setprecision(1) << Energy << "\t"  << 1 << "\t" << std::fixed << std::setprecision(1) << me[0] << "\t" <<  dm[0]*15 <<"\t" << r[0] << "\t" << dr[0]*5 << endl;		
